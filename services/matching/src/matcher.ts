...
export class SmartMatcher {
  ...
  async findMatches(person: any): Promise<any[]> {
  ...
  async comparePeople(person1Id: string, person2Id: string): Promise<any> {
    const session = this.neo4jDriver.session();
    try {
      const result = await session.run(`
  ...
  private calculateSimilarity(str1: string, str2: string): number {
    ...
    const str2Matches: boolean[] = new Array(len2).fill(false);
